...
export function QuizCard({ sessionId, question, resolved, total }: { sessionId: string; question: Question; resolved: number; total: number }) {
  ...
  return <div className="quiz-layout">
    <section>
      ...
      <article className="question-card">
        ...
        <div className="mt-8">
          ...
          : <div><label className="form-label max-w-sm">Your answer<input className="form-input mt-1 text-xl font-bold" inputMode="decimal" value={response} maxLength={response.startsWith("-") ? 6 : 5} disabled={feedback?.correct} onChange={(event) => { setResponse(event.target.value.replace(/\s/g, "")); if (feedback && !feedback.correct) setFeedback(null); }} placeholder="e.g. 3/4 or .75" /></label><p className="mt-2 text-sm text-[var(--muted)]">Use a number, decimal, or improper fraction. No commas, currency, or percent signs.</p></div>}
        </div>
      </article>
    </section>
    ...
    <aside className="feedback-panel">
      ...
      {feedback?.correct && <><span className="feedback-icon correct"><CheckCircle2 className="size-6" /></span><p className="eyebrow mt-5 text-[var(--green)]">Correct</p><h2 className="section-title">{feedback.firstAttempt ? "Mastered." : "You got there."}</h2><p className="mt-3 text-sm leading-6 text-[var(--muted)]">{feedback.message}</p>{feedback.correctAnswers && <p className="mt-4 rounded-lg bg-[var(--green-soft)] p-3 text-sm font-bold text-[var(--green)]">Accepted answer{feedback.correctAnswers.length === 1 ? "" : "s"}: {feedback.correctAnswers.join(", ")}</p>}<div className="mt-6 border-t border-[var(--line)] pt-5"><p className="eyebrow">Explanation</p><QuestionHtml html={feedback.rationaleHtml} className="mt-3 text-sm" /></div><button className="primary-button mt-6 w-full" onClick={next}>{feedback.completed ? "View results" : <>Next question <ArrowRight className="size-4" /></>}</button></>}
    </aside>
  </div>;
}
