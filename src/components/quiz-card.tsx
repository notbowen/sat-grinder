  ...
  return <div className="quiz-layout">
    <section>
      <div className="quiz-topbar">
        <div>
          <p className="eyebrow">Question {resolved + 1} of {total}</p>
          <div className="chips"><span className="chip question-id-chip">ID {question.displayId}</span><span className="chip">{question.section === "math" ? "Math" : "Reading & Writing"}</span><span className="chip">{question.domainName}</span><span className="chip">{question.skillName}</span><span className={`chip ${question.difficulty === "hard" ? "chip-accent" : ""}`}>{question.difficulty}</span>{question.retryCount > 0 && <span className="chip chip-ink">Retry {question.retryCount}</span>}</div>
        </div>
        <button className="icon-btn" onClick={abandon} disabled={abandoning} aria-label="Abandon quiz">{abandoning ? <LoaderCircle className="size-4 animate-spin" /> : <X className="size-4" />}</button>
      </div>
      <div className="progress-track" role="progressbar" aria-valuemin={0} aria-valuemax={total} aria-valuenow={resolved} aria-label="Quiz progress"><div style={{ width: `${Math.round(resolved / total * 100)}%` }} /></div>
      <article className="question-card">
        <div className="question-annotation-hint"><Highlighter className="size-3.5" aria-hidden="true" /><span>Select text to highlight</span></div>
        ...
        <div className="mt-8">
          ...
          : <div className="max-w-2xl"><label className="form-label" htmlFor={`answer-${question.id}`}>Your answer</label><div className="math-answer-row mt-2"><input id={`answer-${question.id}`} className="input math-answer-input" value={response} maxLength={MAX_MATH_RESPONSE_LENGTH} disabled={feedback?.correct} onChange={(event) => { setResponse(event.target.value); if (feedback && !feedback.correct) { setFeedback(null); attemptTimer.resume(); } }} placeholder="e.g. 5/3 or 0.75" inputMode="text" autoCapitalize="none" autoComplete="off" spellCheck={false} /><span className={`math-answer-preview ${response.trim() ? "" : "math-answer-preview-empty"}`} aria-label="Rendered answer">{response.trim() ? <MathExpression value={response} /> : "Preview"}</span></div><p className="small muted mt-2">The rendered version updates as you type. Use up to {MAX_MATH_RESPONSE_LENGTH} characters.</p></div>}
        </div>
      </article>
    </section>

    <aside className="feedback-panel">
      {!feedback && <><p className="eyebrow">Check your work</p><h2 className="section-title">Ready?</h2><p className="feedback-copy">Feedback is instant. Wrong answers can be retried; retries are recorded.</p>{error && <p className="form-error mt-4" role="alert">{error}</p>}<button className="btn btn-primary mt-6 w-full" onClick={check} disabled={loading || !response}>{loading ? <LoaderCircle className="size-5 animate-spin" /> : "Check answer"}</button></>}
      {feedback && !feedback.correct && <><span className="feedback-icon incorrect"><RotateCcw className="size-5" aria-hidden="true" /></span><p className="eyebrow mt-5" style={{ color: "var(--accent)" }}>Keep going</p><h2 className="section-title">Not quite.</h2><p className="feedback-copy">{feedback.message}</p><button className="btn btn-secondary mt-6 w-full" onClick={() => { setFeedback(null); attemptTimer.resume(); }}>Try again</button></>}
      {feedback?.correct && <><span className="feedback-icon correct"><CheckCircle2 className="size-5" aria-hidden="true" /></span><p className="eyebrow mt-5" style={{ color: "var(--good)" }}>Correct</p><h2 className="section-title">{feedback.firstAttempt ? "Mastered." : "Got there."}</h2><p className="feedback-copy">{feedback.message}</p>{feedback.correctAnswers && (question.type === "spr" ? <div className="accepted-math-answer"><span>Accepted answer{feedback.correctAnswers.length === 1 ? "" : "s"}:</span><span className="accepted-math-values">{feedback.correctAnswers.map((answer, index) => <span key={`${answer}:${index}`}>{index > 0 && <span className="accepted-answer-separator">or</span>}<MathExpression value={answer} /></span>)}</span></div> : <p className="accepted-math-answer">Accepted answer{feedback.correctAnswers.length === 1 ? "" : "s"}: {feedback.correctAnswers.join(", ")}</p>)}<div className="rationale mt-6 border-t pt-5" style={{ borderColor: "var(--line)" }}><p className="eyebrow">Explanation</p><QuestionHtml html={feedback.rationaleHtml} className="mt-3" /></div><button className="btn btn-primary mt-6 w-full" onClick={next}>{feedback.completed ? "View results" : <>Next question <ArrowRight className="size-4" aria-hidden="true" /></>}</button></>}
    </aside>
  </div>;
}
