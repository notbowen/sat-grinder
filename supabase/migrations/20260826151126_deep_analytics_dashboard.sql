alter table public.answer_attempts
  add column active_duration_ms integer
  check (active_duration_ms is null or active_duration_ms between 0 and 3600000);
...
begin
  ...
  if window_days is null then
    ...
    trend_granularity := case when window_days = 90 then 'week' else 'day' end;
    trend_interval := case when window_days = 90 then interval '1 week' else interval '1 day' end;
  end if;
...
drop function if exists public.submit_practice_answer(uuid, uuid, text);
...
begin
  ...
  if p_active_duration_ms is not null and (p_active_duration_ms < 0 or p_active_duration_ms > 3600000) then
    raise exception 'The recorded active time is invalid.';
  end if;
  ...
  insert into public.answer_attempts (
    id, session_id, question_id, user_id, attempt_number, response, is_correct, active_duration_ms, created_at
  ) values (
    extensions.gen_random_uuid(), p_session_id, p_question_id, caller_id,
    attempts_before + 1, pg_catalog.btrim(p_response), (grade ->> 'correct')::boolean,
    p_active_duration_ms, now_at
  );
...
revoke execute on function public.submit_practice_answer(uuid, uuid, text, integer) from public, anon;
grant execute on function public.get_dashboard(text, text) to authenticated;
grant execute on function public.submit_practice_answer(uuid, uuid, text, integer) to authenticated;
